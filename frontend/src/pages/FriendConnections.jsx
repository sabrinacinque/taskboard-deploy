...
export default function FriendConnections() {
  ...
  const relationStatus = () => {
    ...
    if (outgoing.some((p) => p.target.id === foundUser.id))
      return "pending_out";
    if (incoming.some((p) => p.requester.id === foundUser.id))
      return "pending_in";
    return "none";
  };
    ...
    <div className="container-fluid background text-white vh-100 p-3 p-md-4 overflow-y-scroll">
      <div className="pt-3 pt-md-5">
        ...
        <p className="text-white mb-2">
          Search by phone number (example <code>3311234567</code>) and click “Add”
          to send a connection request.
        </p>
        <p className="text-white mb-4">
          Once your request is accepted, in "Your Connections" section, you’ll be able to:
        </p>
        <ul className="mt-2">
          <li>Send them a task directly from the app.</li>
          <li>Call them by tapping the phone icon.</li>
          <li>Message them on WhatsApp by tapping the WhatsApp icon.</li>
        </ul>
        ...
        {foundUser && (
          <div
            className="card mb-5 bg-dark text-white border-secondary"
              ...
              <div>
                ...
                <p className="card-text mb-0">
                  ...
                  <small className="text-muted">
                    {foundUser.number || "no phone on record"}
                  </small>
                </p>
              </div>
              <div>
